...
public sealed class RunnerHubClient : IAsyncDisposable
{
    ...
    public RunnerHubClient(IOptions<RunnerOptions> opts, ILogger<RunnerHubClient> logger)
    {
        ...
        _connection = new HubConnectionBuilder()
            ...
            {
                // mTLS: runner presents its enrolled certificate on the connection
        ...
        _connection.Reconnected += id => { _logger.LogInformation("Runner reconnected. Connection: {Id}", id); return Task.CompletedTask; };
        _connection.Closed += ex => { _logger.LogWarning(ex, "Runner SignalR connection closed"); return Task.CompletedTask; };
    }
    ...
    {
        _logger.LogInformation("Connecting runner to control channel...");
        await _connection.StartAsync(ct);
        _logger.LogInformation("Runner connected. ConnectionId: {Id}", _connection.ConnectionId);
    }
    ...
    private Task OnJobDispatched(string jobId, string payloadJson)
    {
        ...
        // Job execution is delegated to the RunnerWorker via a channel
    ...
    private static System.Security.Cryptography.X509Certificates.X509Certificate2 GetRunnerCertificate(string thumbprint)
    {
        if (string.IsNullOrEmpty(thumbprint))
            throw new InvalidOperationException("CS-RUNNER-ERR-001: Runner cert thumbprint is not configured. Run runner enrollment first.");
